...
export default function CatComp({categories} : {categories : Categories[]}) {
  return (
    ...
    <div className='px-0 pt-2 mt-3 border-t flex flex-col items-center justify-center  border-[#13bfe3]'>
     ...
     <Link href={`/category/${category?._id}`} className=' bg-[#13bfe3] md:hidden text-[12px] rounded-xl px-5 w-24 duration-300 hover:bg-transparent border border-[#13bfe3] justify-around     md:py-1 Asimovian cursor-pointer gap-1 flex items-center '>Details<Eye className='w-[16px]'  /></Link>
